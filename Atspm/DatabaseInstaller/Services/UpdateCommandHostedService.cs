...
namespace DatabaseInstaller.Services
{
    public class UpdateCommandHostedService : IHostedService
    {
        ...
        public async Task StartAsync(CancellationToken cancellationToken)
        {
            try
            {
                Console.WriteLine("Starting database migration and setup...");
                Console.WriteLine();
                ...
                Console.WriteLine();
                Console.WriteLine("Database migration and admin seeding completed successfully.");
        ...
        private async Task ApplyMigrationsForAllContexts(CancellationToken cancellationToken)
        {
            ...
            // ConfigContext
            Console.WriteLine("[1/4] Processing ConfigContext...");
            ...
            Console.WriteLine("      ✓ ConfigContext completed");
            Console.WriteLine();

            // AggregationContext
            Console.WriteLine("[2/4] Processing AggregationContext...");
            ...
            Console.WriteLine("      ✓ AggregationContext completed");
            Console.WriteLine();

            // EventLogContext
            Console.WriteLine("[3/4] Processing EventLogContext...");
            ...
            Console.WriteLine("      ✓ EventLogContext completed");
            Console.WriteLine();

            // IdentityContext
            Console.WriteLine("[4/4] Processing IdentityContext...");
            ...
            Console.WriteLine("      ✓ IdentityContext completed");
            Console.WriteLine();

            // Seed roles and claims
            Console.WriteLine("Seeding roles and claims...");
            await RolesAndClaimsDBInitializer.SeedRolesAndClaims(serviceProvider, _config.IdentityConnection);
            Console.WriteLine("✓ Roles and claims seeded");
        }
        ...
        private async Task SeedAdminUserAndAssignRole()
        {
            ...
            // Check if the admin user already exists.
            Console.WriteLine($"Checking if admin user '{_config.AdminEmail}' exists...");
            var adminUser = await userManager.FindByEmailAsync(_config.AdminEmail);
            if (adminUser == null)
            {
                // Create the admin user.
                ...
                if (!createResult.Succeeded)
                {
                    ...
                    Console.WriteLine($"ERROR: Failed to create admin user: {errorMessages}");
            ...
            if (!await userManager.IsInRoleAsync(adminUser, _config.AdminRole))
            {
                Console.WriteLine($"Assigning '{_config.AdminRole}' role to admin user...");
                ...
                if (!roleResult.Succeeded)
                {
                    ...
                    _logger.LogError("Failed to assign admin role: {Errors}", errorMessages);
                    Console.WriteLine($"ERROR: Failed to assign admin role: {errorMessages}");
                    throw new Exception($"Failed to assign admin role: {errorMessages}");
                }
                _logger.LogInformation("Admin user assigned to Admin role successfully.");
                Console.WriteLine($"✓ Admin user assigned to '{_config.AdminRole}' role");
            }
            else
            {
                _logger.LogInformation("Admin user is already assigned to the Admin role.");
                Console.WriteLine($"✓ Admin user already has '{_config.AdminRole}' role");
            }
        }
