...
cmdBuilder.UseHost(hostBuilder =>
{
    ...
    .ConfigureAppConfiguration((h, c) =>
    {
        ...
        // Map --provider command line argument to all ConnectionStrings providers
        var config = c.Build();
        var provider = config.GetValue<string>("provider") ?? config.GetValue<string>("CommandLineOptions:Provider");
        if (!string.IsNullOrEmpty(provider))
        {
            var providerOverrides = new Dictionary<string, string>
            {
                ["ConnectionStrings:ConfigContext:Provider"] = provider,
                ["ConnectionStrings:AggregationContext:Provider"] = provider,
                ["ConnectionStrings:EventLogContext:Provider"] = provider,
                ["ConnectionStrings:IdentityContext:Provider"] = provider
            };
            c.AddInMemoryCollection(providerOverrides);
        }
    })
